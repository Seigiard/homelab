...
- **`do_deploy`/`do_rebuild` в `scripts/docker/_lib.sh` проверяют монтирование** (`check_data_mount`) перед подъёмом сервиса, чей compose ссылается на `DATA_PATH`, если этот путь описан в `/etc/fstab`. Проверка стоит в `do_deploy`, а не в `ensure_service_dirs`: та обрезает строку тома по первому двоеточию и пути вида `${DATA_PATH:-/mnt/data}/public:/srv/public` до неё не доходят вовсе.
