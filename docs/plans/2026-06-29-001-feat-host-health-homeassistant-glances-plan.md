...
- **Имя сервиса нотификации** — **зафиксировано (2026-06-30):** `notify.mobile_app_nothingphone` (legacy mobile_app сервис — принимает `data.priority/ttl`). README ошибочно называл `notify.nothingphone`: это notify-**сущность**, которую philips зовёт через `notify.send_message`; legacy-сервиса с таким именем нет, поэтому первый вариант молча не слал пуш.
- **Ручной «Run» в UI** не передаёт объект `trigger` → шаблоны `{{ trigger.* }}` падают, пуш не уходит. Сообщения читают значения через `states()`, поэтому и Run, и реальный триггер работают.
...
**Status (2026-06-30): выполнено.** U1–U4 + U6 сделаны и проверены на сервере (порт на loopback, интеграция подключена, алерт прислал пуш на телефон, дашборд «Сервер» рисует). U5 — Deferred (обороты нативны). U6: `services/homeassistant/README.md` → «Server hardware monitoring» + ссылка в `ENVIRONMENT.md`.
